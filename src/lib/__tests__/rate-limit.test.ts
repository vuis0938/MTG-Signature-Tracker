...
describe("rateLimit", () => {
  ...
  it("未超限时逐次递减剩余次数", async () => {
    await rateLimit("test:decrement", 3, 60_000);
    const second = await rateLimit("test:decrement", 3, 60_000);
    expect(second.remaining).toBe(1);

    const third = await rateLimit("test:decrement", 3, 60_000);
  ...
  it("超过最大次数后拒绝", async () => {
    ...
    const max = 3;
    for (let i = 0; i < max; i++) {
      const r = await rateLimit(key, max, 60_000);
  ...
  it("时间窗口过后重置计数", async () => {
    ...
    const after = await rateLimit(key, 2, 60_000);
  ...
  it("不同 key 互不影响", async () => {
    ...
    const blockedA = await rateLimit("key:a", 1, 60_000);
  ...
  it("resetAt 返回正确的过期时间戳", async () => {
    const now = Date.now();
    const result = await rateLimit("test:resetat", 5, 10_000);
    expect(result.resetAt).toBe(now + 10_000);
  });

  it("窗口内多次请求 resetAt 保持不变", async () => {
    const key = "test:consistent-resetat";
    const first = await rateLimit(key, 5, 10_000);
    vi.advanceTimersByTime(3_000);
    const second = await rateLimit(key, 5, 10_000);
