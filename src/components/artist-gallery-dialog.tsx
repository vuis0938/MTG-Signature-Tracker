...
export default function ArtistGalleryDialog({
  ...
  return (
    <Dialog
      ...
      className="max-w-3xl flex flex-col h-[min(calc(100svh-2rem),calc(1.4*100vw+27px))] sm:h-[75vh] !max-h-[min(calc(100svh-2rem),calc(1.4*100vw+27px))] sm:!max-h-[75vh] overflow-hidden pr-0"
    >
      ...
      <DialogContent className="flex-1 overflow-y-auto min-h-0 px-6 pb-2">
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 items-start content-start">
            {cards.map((card) => (
              <div
